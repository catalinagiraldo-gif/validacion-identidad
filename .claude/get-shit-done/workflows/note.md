...
- **Global scope**: `D:/validacion-identidad/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `D:/validacion-identidad/.claude/notes/`)
...
2. Glob `D:/validacion-identidad/.claude/notes/*.md` (if directory exists) — global notes
...
Global (D:/validacion-identidad/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `D:/validacion-identidad/.claude/notes/` — works in any directory
