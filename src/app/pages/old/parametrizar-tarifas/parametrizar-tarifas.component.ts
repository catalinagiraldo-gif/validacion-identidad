...
export class ParametrizarTarifasComponent implements OnInit {
  ...
  simAccordionOpen: Record<string, boolean> = { dropi: true, transportadora: false };
  ...
  hasCarrierLevelChanges(carrier: Carrier): boolean {
    ...
    return carrier.iva_enabled !== snap.iva_enabled ||
           ...
           JSON.stringify(carrier.volumen) !== JSON.stringify(snap.volumen);
  }

  hasVolumenChanges(carrier: Carrier): boolean {
    const key = 'carrier-' + carrier.id;
    if (!this.carrierSnapshots[key]) return false;
    const snap = JSON.parse(this.carrierSnapshots[key]);
    return JSON.stringify(carrier.volumen) !== JSON.stringify(snap.volumen);
  }
  ...
  getUnsavedTrayectoNames(): string[] {
    ...
    if (this.hasVolumenChanges(carrier)) names.push('Descuentos Volumen');
    carrier.trayectos
  ...
  private restoreAllSnapshots(): void {
    ...
    if (this.carrierSnapshots[carrierKey]) {
      ...
      carrier.iva_dropi = snap.iva_dropi;
      carrier.iva_carrier = snap.iva_carrier;
      carrier.volumen = JSON.parse(JSON.stringify(snap.volumen));
    }
  ...
  buildChangeSummary(carrier: Carrier, trayectoId: number): { campo: string; anterior: string; nuevo: string }[] {
    ...
    // Volumen changes
    const carrierKey2 = 'carrier-' + carrier.id;
    if (this.carrierSnapshots[carrierKey2]) {
      const snap = JSON.parse(this.carrierSnapshots[carrierKey2]);
      if (snap.volumen && JSON.stringify(carrier.volumen) !== JSON.stringify(snap.volumen)) {
        const oldActive = snap.volumen.find((v: any) => v.active);
        const newActive = carrier.volumen.find((v: any) => v.active);
        changes.push({
          campo: 'Descuento Volumen Vigente',
          anterior: oldActive ? oldActive.range : 'Ninguno',
          nuevo: newActive ? newActive.range : 'Ninguno',
        });
      }
    }
  ...
  private static readonly EMPTY_BREAKDOWN: SimBreakdown = {
    dropiSobre: 0, dropiSeg: 0, dropiIva: 0, totalDropi: 0,
    carrierSobre: 0, carrierSeg: 0, cod: 0, carrierIva: 0, totalTransportadora: 0,
    utilidad: 0, margen: 0,
  };

  get simResultFromActive(): SimBreakdown {
    const carrier = this.singleSelectedCarrier;
    const t = this.activeTrayecto;
    if (!carrier || !t || this.fabSimValor <= 0) {
      return ParametrizarTarifasComponent.EMPTY_BREAKDOWN;
    }

    const valor = this.fabSimValor;

    const dropiSobre = this.calcSobrefleteValue(t.dropi.sobreflete, valor);
    const dropiSeg = valor * t.dropi.seguro / 100;
    const dropiSub = dropiSobre + dropiSeg + t.dropi.incremento;
    const dropiIva = carrier.iva_enabled ? dropiSub * carrier.iva_dropi / 100 : 0;
    const totalDropi = dropiSub + dropiIva;

    const carrierSobre = this.calcSobrefleteValue(t.carrier.sobreflete, valor);
    const carrierSeg = valor * t.carrier.seguro / 100;
    const cod = valor * t.carrier.cod / 100;
    const carrierSub = carrierSobre + carrierSeg + t.carrier.devolucion;
    const carrierIva = carrier.iva_enabled ? carrierSub * carrier.iva_carrier / 100 : 0;
    const totalTransportadora = carrierSub + carrierIva;

    const utilidad = totalDropi - totalTransportadora;
    const margen = totalDropi > 0 ? (utilidad / totalDropi) * 100 : 0;

    return {
      dropiSobre, dropiSeg, dropiIva, totalDropi,
      carrierSobre, carrierSeg, cod, carrierIva, totalTransportadora,
      utilidad, margen,
    };
  }
