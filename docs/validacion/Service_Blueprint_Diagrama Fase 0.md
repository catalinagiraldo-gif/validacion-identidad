...
2. ⚪ Recordatorio por WhatsApp/Correo (CRM): mensaje automático de educación en seguridad que le llega al usuario días después de su primer login, si no verificó desde el Panel Lateral — usa el mismo motivo que el modal (no "retirar dinero", ver nota de la copy corregida arriba): *"Hola 👋 Notamos que aún no verificas tu cuenta en Dropi. Lo necesitamos para confirmar quién eres y mantener la plataforma segura — no toma más de unos minutos."* Es el mismo envío programado que se define en Back stage (punto 2) — aquí es donde el usuario realmente lo ve.
3. 🟣 Sutil: Permite navegación total. Usuario puede ignorarlo y seguir comprando — pero si hace clic en "Transferir Wallet", "Registro de Datos Bancarios", "Solicitud DropiCard", "Datos de Facturación" o "Información de Cuenta" antes de verificarse, salta directo al modal interceptor de Etapa 0.5: mismo trato que un usuario Activo, sin importar que sea Nuevo. — *(flecha horizontal índigo: "FS E0.5")*
...
**Usuarios:** Usuarios Activos (Dropshippers / Proveedores) — y también Usuarios Nuevos, si intentan estas mismas acciones (Transferir Wallet, Registro de Datos Bancarios, Solicitud DropiCard, Datos de Facturación, Información de Cuenta) antes de verificarse en Etapa 0. El disparador es la acción, no la antigüedad de la cuenta: reciben el mismo modal interceptor.

**Tarea** (secuencia horizontal):
1. 🟢 Dropshipper en Módulo Financiero
2. 🟣 Clic: "Transferir Wallet", "Registro de Datos Bancarios", "Solicitud DropiCard", "Datos de Facturación" o "Información de Cuenta" *(Growth Ops: interceptar en "Registro de Datos Bancarios" evita que el usuario configure una cuenta sin validarse — es el paso previo al retiro real. "Datos de Facturación" e "Información de Cuenta" se suman como triggers: editar esos datos también exige verificarse primero)* — *(conector vertical hacia Canales, etiqueta "Intercepta Clic")*
...
2. 🔵 🟦 PENDIENTE — dos motivos distintos, cada uno con su propio mensaje (no es un solo estado):
   - **En revisión de Financiero** (Sumsub ya recibió todo, falta la auditoría tributaria interna): Modal muestra **"Tu verificación sigue en proceso"** — *"Puede tardar hasta 72 horas hábiles. Te avisaremos apenas esté lista — no necesitas hacer nada más."* Botón: **"Entendido"**.
...
🟥 RECHAZADO: Modal muestra **"No pudimos verificar tu identidad"** — *"Por seguridad, restringimos las operaciones de esta cuenta. Si crees que es un error, contáctanos y lo revisamos."* Botón: **"Contactar a soporte"** — luego expulsión (ver Etapa 1). *(Copy y justificación de tono: ver `UX-Writing-Modales-UserPilot.md`)*
...
| 3 | ETAPA 0.5 | Tarea | Clic: "Transferir Wallet", "Registro de Datos Bancarios", "Solicitud DropiCard", "Datos de Facturación" o "Información de Cuenta" | Intercepta Clic | Canales | UserPilot (id `userpilot-fase05`) |
