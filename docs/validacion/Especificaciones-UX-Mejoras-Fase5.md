...
| `2.3` Proceso abandonado | Banner + botón único "Reanudar validación", dispara a los 10 min de inactividad. Mismo copy sin importar cuánto avanzó el usuario. | **Reusar el patrón, corrigiendo el alcance.** El resume **manual** de Fase 5 sí es una mejora real ya existente: el botón "Continuar validación" del estado Incompleto (C6) promete retomar "exactamente donde quedó" (`UX-Writing` §9c) — mantener esa mejora. Pero el **disparo automático por inactividad no existía** en Fase 5: no estaba ni en el blueprint ni en el prototipo, solo el resume manual. Se agregó en esta iteración — blueprint C5 + prototipo `identity-sumsub-modal`: aviso *"¿Sigues ahí?"* a los 10 min de inactividad, con botón único *"Reanudar validación"*, que conserva lo ya tecleado. |
