...
- V5: quita "cuando quieras" del headline. Queda "Verifica tu cuenta" — directo, sin calificar el tiempo. La urgencia real ("ahora") vive solo en el botón ("Verificar ahora"), no en el título.
- V6: headline colaborativo ("Verifiquemos tu cuenta") + body sin ninguna razón concreta ("Así te conocemos mejor y cuidamos tu cuenta como se debe") — más cálido, pero diluía de quién es la responsabilidad de hacerlo y no daba un motivo verificable.
- V7: body con el motivo tomado de `Historia.md` ("se vuelve obligatoria cuando el usuario intenta su primer retiro o transferencia entre wallets") — pero esto vende un *beneficio futuro del usuario* (poder retirar/transferir) que un usuario recién llegado al Home no puede ejercer todavía: no tiene wallet, no ha generado dinero en Dropi. Decirle "retira tu dinero" en ese momento no tiene piso real.
- V8 (actual): cambia el eje del motivo de "beneficio futuro del usuario" a "necesidad presente de Dropi". Ya no dice qué va a poder hacer el usuario después; dice qué necesita Dropi ahora (confirmar quién es, mantener la plataforma segura) — sigue siendo indicativo (el usuario debe hacerlo) y sigue dando un porqué, pero sin prometerle nada que todavía no le aplica.

**Justificación de tono:** el proyecto (`Historia.md`, sección Contexto) es explícito en que la validación "da la percepción de ser obligatoria desde el registro" aunque técnicamente solo se hace cumplir en el primer retiro o transferencia — el banner de Fase 0 existe para instalar esa percepción de obligatoriedad desde el día 1. Por eso el headline sigue sonando a instrucción dirigida al usuario ("Verifica tu cuenta"), no a invitación compartida. Pero el *motivo* que sostiene esa instrucción ya no puede ser "vas a poder retirar tu dinero" (V7), porque a esta altura del onboarding el usuario no tiene wallet ni saldo — ese beneficio todavía no existe para él. El motivo real y honesto en este punto es la necesidad de Dropi ("lo necesitamos para confirmar quién eres"), reforzada por seguridad de la plataforma en general — no la de su dinero, porque todavía no tiene. Sigue sin haber urgencia forzada en el tono (el botón X sigue disponible), y sigue sin mencionar "DropiCard" ni "retiro" — ninguno de los dos existe todavía en la experiencia de un usuario que recién entra al Home.
