...
| Body | Es el paso que necesitas completar para retirar tu dinero o hacer transferencias entre tus wallets. |
...
- V7 (actual): vuelve al imperativo singular ("Verifica", no "Verifiquemos") — la acción es del usuario, no una tarea compartida — y el body da el motivo real, tomado de `Historia.md`: la validación "se vuelve obligatoria cuando el usuario intenta su primer retiro o transferencia entre wallets". El banner adelanta esa razón desde el Home en vez de dar un motivo genérico de "seguridad" sin contenido.

**Justificación de tono:** el proyecto (`Historia.md`, sección Contexto) es explícito en que la validación "da la percepción de ser obligatoria desde el registro" aunque técnicamente solo se hace cumplir en el primer retiro o transferencia — es decir, el banner de Fase 0 existe justamente para instalar esa percepción de obligatoriedad desde el día 1. Por eso el headline debe sonar a instrucción dirigida al usuario ("Verifica tu cuenta"), no a invitación compartida ("Verifiquemos"). El body ya no evita mencionar retiros/transferencias entre wallets — al contrario, nombrarlos es lo que hace el motivo verificable y concreto en vez de una apelación abstracta a "seguridad". Sigue sin haber urgencia forzada en el tono (el usuario puede cerrar el banner con X y seguir comprando), y sigue sin mencionar "DropiCard" — ese producto específico no aparece en `Historia.md` como parte del gatillo real de obligatoriedad, solo "retiro" y "transferencia entre wallets".
